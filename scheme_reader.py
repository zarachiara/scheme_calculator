...
def read_tail(src):
    ...
    try:
        ...
        "*** YOUR CODE HERE ***"
        # problem 2
        if src.current() == ".": # add support to dotted list
            src.pop() # open paren has been popped already, this is for the next one
            next_val = scheme_read(src) 
            if src.pop() != ")": # hint portion: check to see closing parenthesis is as follows
                raise SyntaxError("Expected one element after .")
            return next_val # return expression without dot for a second attribute that may be any Scheme value
        # end of problem 2

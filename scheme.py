...
def apply_primitive(procedure, args, env):
    ...
    if procedure.use_env: # self.use_env boolean flag that indicates whether or not primitive procedure will expect current environment to be passed in as the last argument.  
        args_list.append(env)
    try: 
        return procedure.fn(*args_list) # self.fn implements primitive scheme procedure
    except TypeError: 
...
class Frame(object):
    ...
    def lookup(self, symbol):
        ...
        if symbol in self.bindings: 
            return self.bindings[symbol] # return value of symbol in self.bindings if it exists
        elif self.parent is not None: # lookup symbol if parent exists  
    ...
    def make_call_frame(self, formals, vals):
        ...
        if len(formals) != len(vals):
            raise SchemeError("Number of formals is not equal to number of values")     
...
def do_lambda_form(vals, env):
    ...
    "*** YOUR CODE HERE ***"
    if len(vals.second) > 1: # when function bodies has more than one expression 
        body = Pair('begin', vals.second) # create a pair for body to pass in lambda procedure 
    else:
        body = vals.second.first
    return LambdaProcedure(formals, body, env) # call lambda procedure to verify that procedure has been read correctly by evaluating a lambda expression
...
def do_define_form(vals, env):
    ...
    if scheme_symbolp(target):
        ...
        "*** YOUR CODE HERE ***"
        if scheme_symbolp(target.first): # i think there is a better way of doing this rather than calling scheme_symbol
            env.bindings[target.first] = do_lambda_form(Pair(target.second, vals.second), env)
            return target.first
        # else:
        #     raise SchemeError("bad argument to define") # redudant. Let's fix this  
    else:
...
def do_let_form(vals, env):
    ...
    for pair in bindings: 
        new_env.define(pair[0], scheme_eval(pair[1], env))
...
def do_if_form(vals, env):
    ...
    if scheme_true(predicate): 
        return vals[1] # add scheme_eval if fails later 
    if len(vals) > 2: 
        return vals[2] 
    return okay
...
def do_cond_form(vals, env):
    ...
    for i, clause in enumerate(vals):
        print(clause.first, clause.second)
        check_form(clause, 1)
...
def do_begin_form(vals, env):
    ...
    check_form(vals, 1)
    "*** YOUR CODE HERE ***"
    # if vals is nil or vals.second is nil: 
    #     return vals.first # since the value of the final sub-expression would be vals.first
    # scheme_eval(vals.first, env) # includes the first and last sub-expression
...
def check_formals(formals):
    ...
    for f in formals: 
        if f in parameters: 
            raise SchemeError(str(f) + " repeat formal")
        if not scheme_symbolp(f): 
            raise SchemeError(str(f) + " not valid scheme symbol") 
        parameters.append(f)
